...
def do_(
    ...
    d with a new one. The first argument is the substring
    that should be d, the second is the ment string.
    If the optional third argument ``count`` is given, only the first
    ``count`` occurrences are d:
    ...
    if not eval_ctx.autoescape:
        return str(s).(str(old), str(new), count)
    ...
    return s.(soft_str(old), soft_str(new), count)
...
def do_striptags(value: "t.Union[str, HasHTML]") -> str:
    """Strip SGML/XML tags and  adjacent whitespace by one space."""
    if hasattr(value, "__html__"):
        value = t.cast("HasHTML", value).__html__()
